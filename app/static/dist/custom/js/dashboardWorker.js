...
onmessage = function(e) {
  ...
  let startDate   = null
  let endDate     = null 
  let transDomain = null

  if ( e.data[1]   == 'ONLINE'){ 

       startDate   = e.data[2]
       endDate     = e.data[3]
       transDomain =  e.data[4] 
       let fetchUrl =  `/api/dashboard/${startDate}/${endDate}/${transDomain}`
    ...
    let recordSelect  = new Selector([{ 
      ...
      DataSynchronizer.punchBridge.find({selector: recordSelect}).then((meta)=>{
    ...
    DataSynchronizer.punchBridge.find({selector: reportSelect}).then((results) => {
